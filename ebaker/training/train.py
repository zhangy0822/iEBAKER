...
def train_one_epoch(
    ...
    top_drop_index = int(args.dropratio * model_without_ddp.queuesize)
    top_drop_index = min(max(top_drop_index, 0), model_without_ddp.queuesize - 1)
    sorted_global_sim, _ = torch.sort(model_without_ddp.queuesim_global)
    model_without_ddp.threshold_global = sorted_global_sim[top_drop_index]
    if args.eba_strategy == "split":
        sorted_local_sim, _ = torch.sort(model_without_ddp.queuesim_local)
        model_without_ddp.threshold_local = sorted_local_sim[top_drop_index]
    else:
        model_without_ddp.threshold_local = model_without_ddp.threshold_global
    model_without_ddp.threshold = model_without_ddp.threshold_global
    for i, batch in enumerate(dataloader):
        step = num_batches_per_epoch * epoch + i
        scheduler(step)

        index, images, texts, labels = batch
        all_labels = get_gathered_item(labels.to(device), args)
        all_index = get_gathered_item(index.to(device), args)
        ...
        with autocast():
            with forward_context():
                ...
                image_features, text_features, logit_scale,text_tokens,image_tokens,local2,mlm_scores,mlm_labels,threshold_global,threshold_local  = model(images, texts, text_only=(args.cache_teacher is not None) or args.w_align==0)#
                ...
                if args.w_simcse > 0:
                    ...
                    ssl_loss = args.w_simcse * simcse_contrastive_loss(image_features, text_features,local2,mlm_scores,mlm_labels,epoch,threshold_global,threshold_local,logit_scale=np.exp(2.996))
                else:
                    ssl_loss = 0
        ...
        if args.cache_teacher is not None:
            ...
            # # # # # # # # # # # # # # # # # # 
            if is_master(args):
                cacher.load_batch(all_index, text_features)
